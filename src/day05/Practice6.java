...
for(int index=0;index<=scores1.length-1;index++){
    sum+=scores1[index]; // index번째 요소값을 누적 합계
}
...
선언 코드: */
        ...
        System.out.println("제품명: "); String productName=scan.next();
        System.out.println("구매수량: "); int stockCount=scan.nextInt();
        int check=0;// 0이면 제품 없다 뜻
        for(int index=0;index<=products.length-1;index++){
            if(products[index].equals(productName)){
                if(stock[index]>=stockCount){
                    stock[index]-=stockCount;
                    check=1; break; // 1이면 구매 완료 뜻
                }
                else{
                    check=2;  // 2이면 재고 부족 뜻
                }
        ...
        if(check==0){
            System.out.println("없는 제품 입니다.");} // for 밖에서 제품 없음 출력
        else if(check==1){
