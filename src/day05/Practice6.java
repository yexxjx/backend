...
선언 코드: */
        ...
        for (int index = 0; index <= movieNames.length - 1; index++) { // (1) 0부터 마지막인덱스 이하까지 1씩 증가
            String movie = movieNames[index]; // (2) index번째 영화 이름 가져오기
            System.out.print(movie); // print : 줄바꿈 안 됨
...
선언 코드: */
        ...
        for(int index=0; index<=carNumbers.length-1; index++){
            ...
            int useTime=usageMinutes[index];
            if(useTime<=30){money=1000;}
            else{money=(useTime-30)/10;
                money*=500; // 10분당 500원
